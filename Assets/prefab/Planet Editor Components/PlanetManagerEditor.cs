...
public class PlanetManagerEditor : MonoBehaviour
{
    ...
    private PlanetControllerEditor[] ctrl;
    ...
    void Start()
    {
        ...
        ctrl = new PlanetControllerEditor[planets.Count];
        ...
        foreach(PlanetSceneObject p in planets) {
            ...
            PlanetControllerEditor newPlanetController = pObj.GetComponent<PlanetControllerEditor>();
            newPlanetController.planet = p.planet;
            newPlanetController.id = id;
            obj[id] = pObj;
            sc[id] = p;
            ctrl[id] = pObj.GetComponent<PlanetControllerEditor>();
            if(!p.isStatic) {
    ...
    void FixedUpdate() {
        ...
        for(int i = 0; i < planets.Count; i++) {
            if(!sc[i].isStatic) {
                ...
                float velMagX = -1.0f * Sin( velAngle[i] * timer );
                float velMagY = Cos( velAngle[i] * timer) ;
